...
main(int argc, char *argv[]) {
    ...
    // Create a basic data mask for /any/ kind of bluetooth device
    // address (local and non-local).
    bdaddr_t bdaddr;
    ...
    if (remote) {
        ...
        while (!io_canceled()) {
            do_search(btHIDSocket, &bdaddr, debug);
            sleep(2);
        }
    ...
    int csk = l2cap_listen(&bdaddr, L2CAP_PSM_HIDP_CTRL, L2CAP_LM_MASTER, 10);
    if (csk < 0) {
    ...
    int isk = l2cap_listen(&bdaddr, L2CAP_PSM_HIDP_INTR, L2CAP_LM_MASTER, 10);
    if (isk < 0) {
